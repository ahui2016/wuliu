...
def update_album_pics(
    newpics: list, album_pics: dict, album_path: Path, thumb_size
) -> dict:
    """返回更新後的 album_pics"""
    if len(newpics) == 0:
        return album_pics
...
def update_album_pics_msgp(
    pics: list, album_pics: dict, album_info: dict, album_path: Path
):
    for pic in pics:
        pic_id = pic["ID"]
